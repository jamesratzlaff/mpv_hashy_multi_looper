...
function Timeline:render()
	...
	local hovered_clip = nil
	if (config.opacity.chapters > 0 and (#state.chapters > 0 or state.ab_loop_a or state.ab_loop_b or #state.clips>0)) then
		...
		if #state.chapters > 0 then
			mp.msg.info(utils.to_string(state.chapters));
		end
		if diamond_radius > 0 then
			...
			local function drawChaptersOrClips(stateKey, startKey, endKey)
				local itemsWStart = state[stateKey]
				local hasEndKey = endKey~=nil
				local hovered_item = nil
				if #itemsWStart > 0 then
					-- Find hovered chapter indicator
					local closest_delta = math.huge

					if self.proximity_raw < diamond_radius_hovered then
						for i, itemWStart in ipairs(itemsWStart) do
							local chapter_x, chapter_y = t2x(itemWStart[startKey]), fay - 1
							local cursor_chapter_delta = math.sqrt((cursor.x - chapter_x) ^ 2 +
								(cursor.y - chapter_y) ^ 2)
							if cursor_chapter_delta <= diamond_radius_hovered and cursor_chapter_delta < closest_delta then
									hovered_item, closest_delta = itemWStart, cursor_chapter_delta
								self.is_hovered = true
							end
						end
					end
					...
					if hasEndKey then
						hovered_clip=hovered_item
					else
						hovered_chapter=hovered_item
					end

					for i, itemWStart in ipairs(itemsWStart) do
						if itemWStart ~= hovered_item then 
							draw_chapter(itemWStart[startKey], diamond_radius)
							if hasEndKey then
								draw_ab_indicator(itemWStart[startKey], 'a');
								draw_ab_indicator(itemWStart[endKey], 'b');
							end
						end
						local circle = { point = { x = t2x(itemWStart[startKey]), y = fay - 1 }, r = diamond_radius_hovered }
						if visibility > 0 then
							cursor:zone('primary_click', circle, function()
								mp.commandv('seek', itemWStart[startKey], 'absolute+exact')
							end)
						end
					end
					...
					if hovered_item then
						draw_chapter(hovered_item[startKey], diamond_radius_hovered)
						if hasEndKey then
							draw_ab_indicator(hovered_item[startKey], 'a');
							draw_ab_indicator(hovered_item[endKey], 'b');
						end
			...
			drawChaptersOrClips("chapters","time")
			drawChaptersOrClips("clips","start","end")
	...
	-- Hovered time and chapter/clip
	local function doTimeChapterClipHovering(hovered_item,state_key,start_key,end_key)
		local rendered_thumbnail = false
		if (self.proximity_raw == 0 or self.pressed or hovered_item) and not Elements:v('speed', 'dragging') then
			local cursor_x = hovered_item and t2x(hovered_item[start_key]) or cursor.x
			local hovered_seconds = hovered_item and hovered_item[start_key] or self:get_time_at_x(cursor.x)
			...
			-- Timestamp
			local opts = {
				size = self.font_size, offset = timestamp_gap, margin = tooltip_gap, timestamp = options.time_precision >
			0,
			}
			...
			-- Chapter title
			if config.opacity.chapters > 0 and #state[state_key] > 0 then
				local _, item = itable_find(state[state_key], function(c) return hovered_seconds >= c[start_key] end,
					#state[state_key], 1)
				if item and not (item.is_end_only ~= nil and item.is_end_only) then
					ass:tooltip(tooltip_anchor, item.title_wrapped, {
						size = self.font_size,
						offset = tooltip_gap,
						responsive = false,
						bold = true,
						width_overwrite = item.title_wrapped_width * self.font_size,
						lines = item.title_lines,
						margin = tooltip_gap,
					})
				end
			end
	...
	doTimeChapterClipHovering(hovered_chapter,"chapters","time");
	doTimeChapterClipHovering(hovered_clip,"clips","start");
	return ass
end
